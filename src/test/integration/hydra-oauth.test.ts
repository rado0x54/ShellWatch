 ...
 *
 *   - mediated DCR (/api/hydra/register) enforces the redirect-URI + scope policy,
...
describe("Hydra OAuth surfaces", () => {
  ...
  describe("mediated DCR (/api/hydra/register)", () => {
    it("provisions a public client for an allowed loopback redirect", async () => {
      const res = await fetch(`${appServer.url}/api/hydra/register`, {
        method: "POST",
