...
export function registerHydraRoutes(params: RegisterHydraRoutesParams): void {
  ...
  // blended doc, then DCR against /api/hydra/register and auth against Hydra.
  ...
  app.get("/.well-known/oauth-authorization-server", async () => ({
    ...
    registration_endpoint: `${ext()}/api/hydra/register`,
  ...
  // Lives under ShellWatch's own `/api/hydra/*` namespace (not `/oauth2/*`) so a
  // reverse proxy fronting both ShellWatch and Hydra has an unambiguous owner:
  // everything under `/oauth2/*` routes to Hydra, the mediated DCR endpoint to
  // ShellWatch. Hydra's native DCR stays disabled.
